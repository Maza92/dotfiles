...
from .constants import MATERIAL_CACHE_DIR, TEMPLATES, SAMPLE_WALL, DIRECT_CONFIG_TARGETS
...
class MaterialService(BaseService):
    ...
    def __render_templates(self, colors: dict, dark_colors: dict) -> None:
        for template in os.listdir(TEMPLATES):
            strip_hash = template in DIRECT_CONFIG_TARGETS and DIRECT_CONFIG_TARGETS[template].get("strip_hash", False)
            
            if strip_hash:
                colors_no_hash = {key: value.lstrip('#') for key, value in colors.items()}
                self.render_template(
                    colors=colors_no_hash,
                    dark_mode=user_options.material.dark_mode,
                    input_file=f"{TEMPLATES}/{template}",
                    output_file=f"{MATERIAL_CACHE_DIR}/{template}",
                )
                dark_colors_no_hash = {key: value.lstrip('#') for key, value in dark_colors.items()}
                self.render_template(
                    colors=dark_colors_no_hash,
                    dark_mode=True,
                    input_file=f"{TEMPLATES}/{template}",
                    output_file=f"{MATERIAL_CACHE_DIR}/dark_{template}",
                )
            else:
                # Renderizado estándar con colores originales
                self.render_template(
                    colors=colors,
                    dark_mode=user_options.material.dark_mode,
                    input_file=f"{TEMPLATES}/{template}",
    ...
    def __copy_direct_configs(self) -> None:
        """Copia los archivos de configuración renderizados directamente a sus directorios."""
        for template_name, config in DIRECT_CONFIG_TARGETS.items():
            suffix = "dark_" if user_options.material.dark_mode else ""
            config_dir = os.path.expanduser(config["config_dir"])
            os.makedirs(config_dir, exist_ok=True)
            
            source_path = os.path.join(MATERIAL_CACHE_DIR, f"{suffix}{template_name}")
            target_path = os.path.join(config_dir, config["output_name"])
            
            if os.path.exists(source_path):
                import shutil
                shutil.copy2(source_path, target_path)
                
                if "reload_cmd" in config and config["reload_cmd"]:
                    Utils.exec_sh_async(config["reload_cmd"])
